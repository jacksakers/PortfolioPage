...
const SAMPLE_ITEMS = [
  {
    ...
    date: '2026-01-01',
    description: 'A short description of this entry will appear here.',
    tags: ['Sample'],
    links: [],
    images: [],
  },
  {
    ...
    date: '2025-11-03',
    description: 'Add posts to this section to replace these placeholders.',
    tags: [],
    links: [],
    images: [],
  },
...
export default function SectionsPanel() {
  ...
  const itemConstraints = useMemo(() => (editingId ? [where('sectionId', '==', editingId)] : []), [editingId]);
  const { data: realItems } = useCollection('items', itemConstraints);
  const previewItems = editingId && realItems.length > 0 ? realItems : SAMPLE_ITEMS;
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] items-start">
      ...
      <PreviewChrome label="Section Preview" style={{ backgroundColor: 'var(--color-background)' }}>
